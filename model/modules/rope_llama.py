...
def apply_scaling(freqs: torch.Tensor):
    ...
    low_freq_factor = 1
    high_freq_factor = 4
    old_context_len = 256  # original llama3 length
...
#     print(f"In apply_rotary_emb - x shape: {x.shape}, freqs_cis shape: {freqs_cis.shape}")
#     xshaped = x.float().reshape(*x.shape[:-1], -1, 2)
#     print(f"xshaped shape: {xshaped.shape}")
#     print(f"Trying to reshape freqs_cis to: [1, {xshaped.size(1)}, 1, {xshaped.size(3)}, 2]")
...
def apply_rotary_emb(x, freqs_cis):
   ...
   xshaped = x_transposed.float().reshape(*x_transposed.shape[:-1], -1, 2)
   actual_seq_len = x_transposed.size(1)
   freqs_cis = freqs_cis[:actual_seq_len]
   freqs_cis = freqs_cis.view(1, actual_seq_len, 1, xshaped.size(3), 2)
   x_out2 = torch.stack(
       [
           xshaped[..., 0] * freqs_cis[..., 0] - xshaped[..., 1] * freqs_cis[..., 1],
           xshaped[..., 1] * freqs_cis[..., 0] + xshaped[..., 0] * freqs_cis[..., 1],
